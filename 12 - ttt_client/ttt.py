...
def StartGame(name):
	...
	dictionaryOfGames[gameId] = Game(name, gameId)
	return gameId
...
class myHandler(BaseHTTPRequestHandler):
	...
	def get_response(self):
		...
		if splittedRequest[0] not in ("start", "status", "play", "list", "update"):
			return HTTPStatus.BAD_REQUEST, bytes(json.dumps({}), "utf-8")
		
		if splittedRequest[0] == "update":
			id = int(splittedRequest[1].split("=")[1])
			if(id not in dictionaryOfGames):
				return HTTPStatus.BAD_REQUEST, bytes(json.dumps({}), "utf-8")
			if(dictionaryOfGames[id].available == False):
				return HTTPStatus.BAD_REQUEST, bytes(json.dumps({}), "utf-8")
			dictionaryOfGames[id].available = False
			return HTTPStatus.OK, bytes(json.dumps({}), "utf-8")
			
		if splittedRequest[0] == "list":
			listToReturn = []
			for key, value in dictionaryOfGames.items():
				if(value.available == True):
					listToReturn.append({"name" : value.name, "id" : value.id})				
			return HTTPStatus.OK, bytes(json.dumps(listToReturn), "utf-8")
